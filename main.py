    ...
    def on_handle_context(self, e_context: EventContext):
        ...
        if clist[0].startswith(trigger_prefix):
            ...
            for i,summary in enumerate(reversed(summarys)):
                query += summary + "\n----------------\n\n"
            prompt = "你是一位群聊机器人，聊天记录已经在你的大脑中被你总结成多段摘要总结，你需要对它们进行摘要总结。严格按照以下按步骤执行。1. 需要对聊天记录进行简明扼要的总结，第二如果发现报告下载链接，提炼出来报告的下载链接指示，第三用列表的形式将总结和下载链接汇总输出。\n"
